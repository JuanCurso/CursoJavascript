...
class Dni {
        ...
        esDNIValido () {
            return this.letra == this.calculaLetra();
        }

        static esDNIValido2 (miDni) {

            return miDni.letra == miDni.calculaLetra();
        }
      
}

function validarDni(){
    const ndni = document.getElementById("numeroDNI").value;
    const ldni = document.getElementById("letraDNI").value;
    const resultadoValidacion = document.getElementById("resultadoValidacion");


    let miDNI = new Dni(ndni, ldni);

    if (Dni.esDNIValido2(miDNI)) 
        resultadoValidacion.innerHTML = "El DNI es valido.";
    else
    resultadoValidacion.innerHTML = "El DNI no es valido.";
